		...
		namespace ui {
			...
			void renderUI() {
				...
				if (simulationRunning && displayGeneration) {
					renderText(
						"Generaciones pariente " + std::to_string(generation),
						gs::Vec2f(500.0f, 80.0f), 1.5f, gs::Color::White, 1.0f
					);
					renderText(
						"Intentos del camba " + std::to_string(attempt),
						gs::Vec2f(500.0f, 120.0f), 1.5f, gs::Color::White, 1.0f
					);
					renderText(
						"Mejor intento :D " + std::to_string(static_cast<int>(
				...
				if (displayInfoMenu) {
					renderText(
						...
						\n\n F1 - Pausas/Despausar simulación\
						\n F2 - Cambiar Punto de Vista\
						\n F3 - Mostrar simulación\n  Información\
						\n F4 - Reiniciar simulción\
						\n F5 - Cambiar fotogramas\
						\n F6 - Fin generacion\n  Temprano\
						\n F11 - Entrar/Salir\n  PantallaCompleta\
						\n\n Espacio - Saltar\
						\n Escape - Cerrar App\
						\n H - Esconder Menu\
						\n\nControles del Editor\
						\n\n Q - Entrar modo editor\
						\n Teclas de flecha - Alterar\n  Tamaño del nivel\
						\n R - Cambiar Objeto\
						\n Enter - Cambiar Nivel\
						\n + - Aumentar tono del fondo\n  Hue",
